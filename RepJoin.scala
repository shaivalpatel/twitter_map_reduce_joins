...
object RepJoin {
  ...
  def main(args: Array[String]) {
    ...
    val broadcastStateData = spark.sparkContext.broadcast(left.collectAsList())
    val broadcastSchema = spark.sparkContext.broadcast(left.schema)
    val DF = spark.createDataFrame(broadcastStateData.value, broadcastSchema.value)
    val rightext = tf.map({
      case x => (x.split(",")(1),x.split(",")(0) )
    })
    val right = rightext.toDF("follower", "following")
    val twopath = right.join(DF,"follower" )
